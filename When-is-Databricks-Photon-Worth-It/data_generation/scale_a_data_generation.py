from pyspark.sql.functions import col, expr, rand, round

# SCALE FACTOR A (~100M users, ~1M products, ~500M transactions)

# Users
users_bronze_scale_a = (
...
users_bronze_scale_a.write.mode("overwrite").format("delta").saveAsTable("bronze.users_bronze_scale_a")

# Products
products_bronze_scale_a = (
...
products_bronze_scale_a.write.mode("overwrite").format("delta").saveAsTable("bronze.products_bronze_scale_a")

# Transactions
transactions_bronze_scale_a = (
    spark.range(500_000_000)
    .withColumnRenamed("id", "transaction_id")
    .withColumn("user_id", (rand() * 100_000_000).cast("long"))
...
transactions_bronze_scale_a.write.mode("overwrite").format("delta").saveAsTable("bronze.transactions_bronze_scale_a")
